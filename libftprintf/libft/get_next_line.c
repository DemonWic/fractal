...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2019/11/19 12:49:45 by ahintz            #+#    #+#             */
/*   Updated: 2019/11/19 12:50:24 by ahintz           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int			ft_treatment(int nbread, t_gnl *new, char **line)
{
	...
	if (nbread < BUFF_SIZE && ((new->content_i) ==
		(size_t)ft_strlen(new->content)))
		return (0);
	i = ft_strmyccpy(line, new->content, '\n', new->content_i);
	if (i < (size_t)ft_strlen(new->content))
		new->content_i = i;
	else
	{
...
int					get_next_line(const int fd, char **line)
{
	...
	nbread = ft_treatment(nbread, new, line);
	return (nbread);
}
